...
### Paint attributes
**Converter › Paint attributes…** opens a floating palette over the map (drag it by its title bar; **Esc** or × closes it). While it is open the flag overlay is forced on over whichever layer you are looking at — switch between minimap, height, tile or water to trace against — left-drag paints, and right-drag (or middle-drag) moves the map, as everywhere else; the wheel still zooms.
...
- **Batch paint, by texture name** — paints the whole map in one go with the mapper’s trick: every tile whose ground texture file name starts with `stone` gets Block, every one starting with `tile` gets Safezone (prefixes are editable, comma-separated, case ignored, folders ignored). Block and Safezone each have their own **Add**, so one can be run without touching the other. Add only ever sets the flag; nothing is cleared — remove flags by hand with the **Clear** mode. It needs the map’s textureset, and the two **Add** buttons stay disabled until one is loaded: the section shows the file name in use (`✓ metin2_c1.txt · 12 textures`) or, while it is missing, the name `setting.txt` asks for. It is picked up by itself when it sits in the map folder; otherwise press **load…** or drop the `.txt` (or the whole textureset folder) onto the palette — or anywhere on the page while the palette is open. A textureset loaded with the picker or by drag and drop is remembered by the workspace (Chrome / Edge) and loaded again when you reopen it; it belongs to that one map, so it is not carried over to other maps.
- **Batch paint, Water** — a third row with its own **Add**, which needs no textureset: an attribute cell gets the Water flag when it lies under a water cell of `water.wtr` **and** its ground is below that water plane. A water plane is a flat sheet that usually reaches past the shore and under islands, so the ground height is read from `height.raw` (interpolated at the centre of each 1 m cell) and compared with the plane’s height; shores and islands sticking out of the water are left alone, and the report says how many cells were skipped for that. A sector without `height.raw` falls back to `water.wtr` alone. It never clears the flag, and it never creates or changes water.
- Each stroke (and each batch run) is one undo step. Edits are written into the map’s `attr.atr` files, so **Export map** and **Generate server_attr** use them; the palette reminds you when `server_attr` is out of date and has the button to regenerate it.
...
- A compass rose at the top centre (N NE E SE S SW W NW) turns with the camera: the label at the top is the direction you are looking, north being the top of the 2D view. **Tools › Compass in 3D Preview** switches it off and on (on by default, remembered in the workspace view settings).
- Objects show their heading in 3D (a short tick on every object, the long arrow on the selected ones, both following **Direction arrows**). The selected object of the Objects tab carries a gizmo that acts on the whole selection: drag the **pin** to move it over the terrain (Z follows the ground, so its offset is kept), drag the **vertical arrow** to raise or lower it (Z and the height bias change together; Shift = 10 cm steps; double-click = back on the ground, bias 0), drag around the **ring** to turn it (roll, in whole degrees since the client truncates the rest; Shift snaps to 15°) — with several objects selected the ring sits on their common centre and they turn around it as one block, positions included, like the WorldEditor's pivot rotation. Each drag is one undo step and the form follows live; a drag that starts anywhere else still orbits.
- Clicks work as in 2D: on the Regen tab a click on empty ground places a spawn at the terrain point under the cursor (with a group selected it only drops the group), **Ctrl+click** a pin adds it to the selection or takes it out, **Ctrl+drag** draws a box on the screen and adds the pins inside, **Ctrl+Alt+drag** removes them. While Ctrl is held a drag never orbits.
- A selected `g` / `r` spawn shows its pack / set card docked in the bottom-left corner of the view, above the zoom buttons — in 2D and in 3D alike, so it never hides the spot being looked at nor runs out of the view. Click the card to fold it down to its title line and back; the choice is shared by both views and remembered in the workspace view settings.
- Drag the pin of a selected spawn to move it over the terrain (the whole selection moves with it, one undo step). The selected spawn of the Regen tab also shows the same range handles as in 2D (edges and corners, standing on the ground): drag one to set Sx / Sy from the terrain point under the cursor, one undo step per drag.
- **Tools › Walk mode in 3D Preview** (or **G**) puts a dummy character on the ground at the camera target and brings the camera behind it: **W / S** walk along the camera direction, **A / D** strafe, **Q / E** turn the camera, **Shift** runs (5 and 15 m/s), the wheel zooms down to 4 m, dragging still orbits. The character follows the terrain and is never blocked; the status line gives its position, height, sector and the attribute flags under its feet (BLOCK, WATER, …). **G** or **Esc** leaves and restores the previous zoom and tilt.
- True vertical scale (`raw × HeightScale`). Only the sectors around the camera target are drawn (up to 7 × 7), so a very large map is never shown whole. The grid and the ruler are not drawn in 3D. Needs WebGL.
- Regen spawns and objects show as pins, by the same rules as in 2D: the current regen file on the Regen tab (plus the other three with **Highlight all regens**), the objects on the Objects tab. Same colours, selection in white, direction arrows, mob names (selected spawns always; all of them while no more than 120 are in view) and spawn ranges laid over the ground. A spawn stands on the ground; an object stands at its own Z, with a dashed line down to the ground when it floats or is buried. A pin behind a hill is drawn faded. With no paint palette open, a click (not a drag) on a pin selects it, **Ctrl**+click adds or removes it from the group, a click on empty ground clears the selection on the Objects tab and places a spawn on the Regen tab (see below); Del, arrow-key nudge, copy / paste and the side panel work on the selection as usual. Double-clicking a row in the side list moves the camera onto it and comes closer if it was far away, like the 2D focus. Placing and dragging markers stays in 2D.
...
- Range handles are hidden while a group is selected — unless every selected spawn stands on the same spot with the same Sx / Sy (mob lines stacked on one area): then the handles stay and a drag resizes them all as one, in 2D and in 3D, as a single undo step. Also, the move-to-another-sector prompt is only asked for single objects (the position warning still flags the rest).
...
- **About MapForge** — credits (author and contributors) and the links to the [web app](https://martysama0134.github.io/M2-MapForge) and the [repository](https://github.com/martysama0134/M2-MapForge); closes with **Enter** or **Esc**.
