...
- Opening a map folder creates a workspace automatically (named after the folder; opening the same folder again reuses it). Loading a regen folder, dropping spawn files or loading `mob_names.txt` links those to the current workspace. Mob names stay loaded when you open another map, and they are linked to that map’s workspace as well, so every workspace you used them with reloads them on its own. **Unload mob names** also takes the file off the open workspace.
