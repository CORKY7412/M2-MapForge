...
- Objects show their heading in 3D (a short tick on every object, the long arrow on the selected ones, both following **Direction arrows**). The selected object of the Objects tab carries a gizmo that acts on the whole selection: drag the **pin** to move it over the terrain (Z follows the ground, so its offset is kept), drag the **vertical arrow** to raise or lower it (Z and the height bias change together; Shift = 10 cm steps; double-click = back on the ground, bias 0), drag around the **ring** to turn it (roll; Shift snaps to 15°) — with several objects selected the ring sits on their common centre and they turn around it as one block, positions included, like the WorldEditor's pivot rotation. Each drag is one undo step and the form follows live; a drag that starts anywhere else still orbits.
